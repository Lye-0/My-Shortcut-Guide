...
## 0.1.3 UI修正 (2026-09-16)

- Releaseビルド: 警告0 / エラー0。Core 15件、Windowsフォーム6グループ成功。
- 本物のマウスドラッグでセクション01とショートカット01を各3番目へ移動。画面表示と検証用保存結果が02,03,01の順になることを確認。実ユーザーデータは未変更。
- 追加フォームにセクション選択なし、保存後も渡したセクションIDを保持。
- フォーム実画面で角丸入力、説明欄の独自スクロール、区切り線と青い結果表示を確認。
- 並べ替え後のJSON保存・復元、境界・無変更・不正位置をCoreテストで確認。
- ドラッグはマウスメッセージのみで動作。常駐タイマーや監視を追加していない。
- アイコンは図形から生成し、内部単色と透明な外側を確認。

## 0.1.4 クリック範囲の修正

実WinFormsコントロールへのマウスメッセージで、選択済みでも一覧下の空白・ブロック間の隙間・右端余白で編集イベントが発生しないこと、ブロック内のダブルクリックでのみ発生すること、マウス捕捉が残らないことを確認。並べ替え無効時も検証。Windowsテスト7グループ成功。Releaseビルド警告0・エラー0。

## 0.1.5 空白クリックの視覚反応

実エディターのイベント配線で、選択済み→空白クリック→選択なし・一覧フォーカスなし・マウス捕捉なし・編集削除無効を検証。ブロックをクリックすると選択とフォーカスが戻ることも確認。並べ替え有効・無効の両状態で成功。Windowsテスト7グループ成功、ビルド警告0・エラー0。
