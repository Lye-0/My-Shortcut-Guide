...
## 0.1.2 共通UI部品（2026-09-16）

- 確認・通知・エラーはアプリの暗色ダイアログを使用。削除の実行ボタンは「削除する」、初期のEnterとEscはキャンセル側。
- 選択欄と候補一覧は独自描画。候補行の余白、現在値のチェック、選択・ホバー、開閉矢印を統一。
- 一覧、説明欄、フォームのスクロール領域、選択候補に共通のスクロールバーを使用。ホイール、ドラッグ、ページ移動とWindowsのスクロール量設定に対応。
- Windowsフォームテスト6群成功。確認の結果、保存、フォーカス、80件/80行の範囲・末尾、候補の確定・取消・スクロールを含む。
- 実画面で40件の候補一覧をホイールで移動し、つまみのドラッグで40件目まで到達。確認ダイアログ、説明欄、候補の展開状態を目視確認。
- 再描画時のスクロールバー更新で発生した描画ループを検出し、不要なスタイル更新と再描画通知を除去。
- キーボードの確定・取消はコントロールのキーイベントテストで確認。スクリーンリーダー全般、タッチ入力、OSファイル選択画面の外観は網羅検証していません。
