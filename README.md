...
![My Shortcut Guideの編集画面。左側にセクションと共通操作、右側にショートカット一覧](docs/images/editor.jpg)

掲載画像は実際の編集画面です。セクション名・登録内容は利用例です。
...
**確認された原因の一例**：開発ツール経由とエクスプローラー経由で起動した際、Windowsの仮想化によって同じパス表記でも別の保存領域を読んでいた事例があります。開発ツール経由で起動したことがある場合は、両方のデータを退避して比較してください。復旧後はエクスプローラーから起動して確認します。[調査内容と復旧時の注意](docs/startup-context.md)
...
開発ツール側からは登録が見えていても、通常のWindows側には登録が存在しなかった事例を確認しました。エクスプローラーから本体を起動し、設定で自動起動をオンにして保存してください。通常のWindows側への登録とバックグラウンド起動は確認済みですが、この事例の実際の再ログインによる確認は未実施です。[起動経路による違い](docs/startup-context.md)
